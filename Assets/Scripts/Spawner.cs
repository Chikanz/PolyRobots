...
public class Spawner : MonoBehaviour
{
    ...
    //Hack to override rotation since they don't fit nicely on the pedestal
    [Serializable] public class RotMap : SerializableDictionary<string, float> {}
    public RotMap rotMap;
    ...
    private async void Yeet()
    {
        ...
        for (int i = 0; i < platforms.Count; i++)
        {
            ...
            var m = await APIManager.instance.MakeModel(models[i], platformObjWidth * 0.9f, false);
            ...
            //override rot
            if(rotMap.Contains(models[i1].Id)) newPiv.transform.rotation = Quaternion.Euler(0,rotMap[models[i1].Id], 0);
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                var t = plane[i, j];
                if (t == null) continue;
                var scaleAnim = t.GetComponentInChildren<ScaleUp>();
                scaleAnim.transform.localScale = Vector3.zero;
            }
        }
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                ...
                if (t == null) continue;
                var scaleAnim = t.GetComponentInChildren<ScaleUp>();
                scaleAnim.animationDelay = (new Vector2(i, j) - center).magnitude * spawnerDelaySpacing;
